...
/** ========= SUPABASE CLIENT (magic link, RLS-safe) ========= */
...
/** ========= THEME TOKENS (Dusty rose + Deep raspberry) ========= */
const tokens = {
...
/** ========= DEMO SEED (so UI isn’t empty on first run) ========= */
const seed = [
...
export default function App(){
  ...
  // local days (seed until server loads)
  const [days,setDays]=useState(seed);
  const [loaded,setLoaded]=useState(false);

  // pull server entries once logged in
  useEffect(()=>{ (async()=>{
  ...
  const series = last7.map(d=>({ d, s: scoreLife(d) }));
  const today = series[series.length-1] || { d: days[days.length-1], s: scoreLife(days[days.length-1]||{}) };
  ...
  // tiles data
  const physSeries = series.map(x=>x.s.phys), mentSeries = series.map(x=>x.s.ment),
        workSeries = series.map(x=>x.s.work), joySeries  = series.map(x=>x.s.joy), finSeries = series.map(x=>x.s.fin);
  ...
  return (
    <div className="min-h-screen w-full p-4 sm:p-6 lg:p-8" style={{ background: tokens.bg, color: tokens.text, fontFamily: "ui-sans-serif, system-ui" }}>
      ...
      <div className="fixed top-2 right-2 z-50">
        <button onClick={()=>supabase.auth.signOut()} className="rounded-full px-3 py-1 text-xs border" style={{ borderColor: tokens.primaryDark, color: tokens.text }}>Sign out</button>
      </div>
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        ...
        <div className="flex items-center gap-3 ml-auto">
          ...
          <button className="rounded-full px-3 py-1 text-sm border" style={{ borderColor: tokens.primaryDark, color: tokens.textSecondary }}>📅 Today</button>
        </div>
      </div>
      ...
      <div className="mt-5 grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
        <Tile title="Physical"  value={today.s.phys} subtext={todaySub.physical}  series={physSeries} />
        <Tile title="Mental"    value={today.s.ment} subtext={todaySub.mental}    series={mentSeries} />
        <Tile title="Work"      value={today.s.work} subtext={todaySub.work}      series={workSeries} />
        <Tile title="Joy"       value={today.s.joy}  subtext={todaySub.joy}       series={joySeries} />
        <Tile title="Financial" value={today.s.fin}  subtext={todaySub.financial} series={finSeries} />
      </div>
